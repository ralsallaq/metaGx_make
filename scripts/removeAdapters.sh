...
if [ ! -f $analysisDir/$sname.noAdpt.fq.gz ]; then
    cutadapt --interleaved -j $ncpus  -a file:$adaptersF -A file:$adaptersF -o $analysisDir/$sname.noAdpt.fq.gz $file 
fi

#echo "create symbolic links to these files"
#if [ -f $analysisDir/$sname.noAdpt.fq.gz ]; then
#unlink $scratch/$sname.temp.fq.gz; ln -s $analysisDir/$sname.noAdpt.fq.gz $readScratch/$sname.temp.fq.gz 
#fi
}
export LSB_JOB_REPORT_MAIL="N"
export -f func1
bsub -q $que  -n $ncpus -R "span[hosts=1] rusage[mem=10000]" -P microbiome -J "remAdapt[1-$nSamples]" -oo "$readScratch/remAdapt_%I.log" "func1 \$LSB_JOBINDEX '$files' '$adaptersF' '$analysisDir' '$ncpus'"
